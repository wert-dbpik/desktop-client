...
import ru.wert.tubus.chogori.entities.drafts.commands.ServiceDeleteDrafts;
...
public class Folder_DeleteCommand implements ICommand {
    ...
    public void execute() {
        ...
        for(Folder item : items){
            draftsToDelete.addAll(ChogoriServices.CH_QUICK_DRAFTS.findAllByFolder(item));
        }
    ...
    private void deleteFolder(int row) {
        for (Folder item : items) {
            try {
                ChogoriServices.CH_QUICK_FOLDERS.delete(item);
