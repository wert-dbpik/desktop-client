    ...
    public boolean pastePossible(String str){
        ...
        for(String s : pasteData){
            ...
            if(clazz.equals("PG") && !pgPK){
                ...
                ProductGroup pastedPG = ChogoriServices.CH_PRODUCT_GROUPS.findById(pastedItemId);
            ...
            if(selectedItem == null) { //Ничего не выделено
                ...
                switch (clazz) {
                    case "DR":
                        return;
                    case "PG":
                        ProductGroup pg = ChogoriServices.CH_PRODUCT_GROUPS.findById(pastedItemId);
                        pg.setParentId(selectedItem.getId());
                        ChogoriServices.CH_PRODUCT_GROUPS.update(pg);
                        selectedItems.add(pg);
                        break;
                    case "F":
                        Folder folder = ChogoriServices.CH_QUICK_FOLDERS.findById(pastedItemId);
                        folder.setProductGroup(ChogoriServices.CH_PRODUCT_GROUPS.findById(selectedItem.getId()));
                        ChogoriServices.CH_QUICK_FOLDERS.update(folder);
                ...
                if (selectedItem instanceof ProductGroup) { //Если при вставке выделена папка с комплектами
                    ...
                    switch (clazz) {
                        ...
                        case "F":
                            Folder folder = ChogoriServices.CH_QUICK_FOLDERS.findById(pastedItemId);
                            folder.setProductGroup(ChogoriServices.CH_PRODUCT_GROUPS.findById(selectedGroup.getId()));
                            ChogoriServices.CH_QUICK_FOLDERS.update(folder);
                            selectedItems.add(folder);
                            break;
                    }
                    ...
                    if (clazz.equals("DR")) {
                        ...
                        Folder folder = ChogoriServices.CH_QUICK_FOLDERS.findById(pastedItemId);
                        folder.setProductGroup(ChogoriServices.CH_PRODUCT_GROUPS.findById(tableView.getUpwardRow().getValue().getId()));
                        ChogoriServices.CH_QUICK_FOLDERS.update(folder);
                        selectedItems.add(folder);
